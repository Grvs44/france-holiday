# France Holiday

A simple PWA containing English and French phrases, with French pronounciation

## Build

Requirements: Node.js and npm

- Install dependencies:
  ```sh
  npm i
  ```
- Run local development server with `npm start`
- Build project with `npm run build` and preview production build with `npm run preview`

## Sources

- Speech synthesis code adapted from [MDN](https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API/Using_the_Web_Speech_API#speech_synthesis)
- Phrases from [Linguanaut](http://www.linguanaut.com/english_french.htm)
